...
| `ai-model/` | 주유소/시설/공시지가 기반 격자 데이터와 AI target/model 생성 | 단계별 README에 feature, target, 학습 결과 상세 |
...
| `ai-model/04` | 격자별 적정가격 예측 모델 학습, 검증, 2026 출력 생성 |
...
AI 04의 LSTM 기반 모델을 최종 모델로 채택합니다. 다만 validation에서 단순 “전일 가격 유지” baseline보다 낮은 성능을 보였고, 2026년 test에서는 전국 적정가격 레벨 변화가 큰 구간에서 오차가 크게 확대되었습니다. 따라서 최종 산출물을 사용할 때는 모델이 “전날까지의 격자 시계열과 공간 context만으로 다음 날 격자 적정가격을 예측한 결과”라는 점과, 전국 단위 급격한 레벨 변화에는 취약하다는 한계를 함께 해석해야 합니다.
