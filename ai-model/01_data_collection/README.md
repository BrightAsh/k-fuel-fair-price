...
격자화와 영향력 feature 생성은 이 단계에서 하지 않습니다. AI Model 02는 격자화 전 파생 CSV를 만들고, 주유소 개수/주변 영향력/시설 영향력은 AI Model 03에서 생성합니다.
...
주유소/시설/공시지가는 AI Model 02 파생 데이터 생성 단계와 직접 연결됩니다.
...
AI Model 02에서 시설 좌표를 검증/보강하고, AI Model 03에서 시설 count/influence를 만들려면 최종적으로 아래 형태의 좌표 포함 파일이 있어야 합니다.

```text
상표, 대상, 경도, 위도
```

원문 코드에서는 시설 목록을 만든 뒤 Google Maps 기반 좌표 보강을 통해 `1 facility_location_data_final.csv`를 만들고, 격자화 단계에서 그 파일을 읽어 시설 feature를 생성했습니다. 새 구조에서는 AI Model 02가 같은 역할의 좌표 포함 CSV를 `DATA_COLLECTION_PATH/derived_data/facility_location_data_final.csv`로 정리합니다.

## Responsibility Boundary

| 항목 | AI Model 01에서 처리 | AI Model 02에서 처리 | AI Model 03에서 처리 |
|---|---|---|---|
| 시설 목록 수집 | O | - | - |
| 시설 좌표 보강 | 선택 | O | - |
| 시설 개수 feature | - | - | O |
...
공시지가 격자 결합, 운영 자동수집 검토 메모, 산출물 계약 점검 셀은 01번 노트북 본문에서 제외하거나 별도 문서로 관리합니다. 공시지가 자체는 수동 파일로 준비하되, 형식 검증은 AI Model 02에서, 격자 패널 결합은 AI Model 03에서 수행합니다.
