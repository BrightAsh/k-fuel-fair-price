...
AI 모델용 최종 격자 패널을 만들기 전, `data collection/`의 수집 산출물을 격자화 가능한 형태로 정리하는 단계입니다. 이 단계는 직접 학습 데이터를 만들지 않고, 02 단계가 사용할 주유소/시설/공시지가/land grid 기반 입력을 생성합니다.
...
| 입력 | 위치 | 용도 |
|---|---|---|
| 환율 | `fx_usdkrw/final/fx_usdkrw_*.csv` | 전국 일별 feature 후보 |
| 원유 | `crude/final/crude_*.csv` | 원유 가격 feature 후보 |
| 국제 제품가격 | `intl_products/final/intl_products_*.csv` | 국제 제품 가격 feature 후보 |
| 전국 평균 가격 | `retail_avg/final/retail_avg_*.csv` | 전국 일별 feature 후보 |
| 브랜드 가격 | `brand_price/final/brand_gasoline_*.csv`, `brand_diesel_*.csv` | 전국 일별 feature 후보 |
| 유류세 | `fuel_tax_trend/final/gasoline_tax_trend_*.xls`, `diesel_tax_trend_*.xls` | 전국 일별 feature 후보 |
| 정유사 주간 공급가격 | `refinery_weekly_supply/final/refinery_weekly_supply_prices_by_product_*.csv` | 전국 일별 feature 후보 |
| 주유소 가격/메타 | `gas_station_prices_by_region/final/{region}/gasoline.csv`, `diesel.csv`, `metadata__latlon.json` | 주유소 좌표, 가격 가능 일자, 속성 이력 |
| 수동 시설 | `z_pa_facility/final/facility_data.csv` | 시설 좌표/유형 |
| 공시지가 | `official_land_price/final/*.csv` | 500m 격자별 공시지가 snapshot |

`gas_station_prices_by_region`은 대용량이라 Git에 올리지 않아도 됩니다. Colab Drive의 `data collection/` 아래에 있으면 노트북이 직접 사용합니다.
...
`z_pa_facility/final/facility_data.csv`를 읽어 시설 유형을 표준화하고 주소 좌표를 보강합니다.
...
```text
data collection/derived_data/korea_land_grid_500m.parquet
data collection/derived_data/station_location_history.csv
data collection/derived_data/station_attribute_history.csv
data collection/derived_data/facility_points.csv
data collection/derived_data/official_land_price_grid.csv
```
