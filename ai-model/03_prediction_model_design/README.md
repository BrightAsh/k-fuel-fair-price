# 03. Prediction Model Design

AI Model 02에서 생성한 500m 격자 일별 패널을 입력으로 사용해, t-1까지의 시계열만으로 target date의 격자 가격 변동을 예측하는 단계입니다. 현재 모델 학습이 진행 중이므로 최종 성능 지표와 결과 해석은 아직 확정하지 않습니다. 이 README는 코드 내용 기준으로 입력, target, 학습 데이터 구성, cache, 모델, 예상 산출물을 정리합니다.

## 실행 파일

| 파일 | 역할 |
|---|---|
| `03_prediction_model_design.py` | 로컬/PyCharm/Anaconda 기준 학습 스크립트 |
| `environment-conda.yml` | 실행 환경 정의 |

이 단계는 Colab mount나 Google Drive fallback을 사용하지 않습니다. repo 루트를 working directory로 두고 실행하는 구조입니다.

## 실행 환경

`environment-conda.yml` 기준입니다.

| 항목 | 값 |
|---|---|
| Python | 3.12 |
| 주요 패키지 | `numpy`, `pandas`, `scikit-learn`, `pyarrow`, `duckdb` |
| PyTorch | `https://download.pytorch.org/whl/cu126` index의 `torch` |

실행 예시:

```powershell
cd C:\Users\user\Desktop\산업부\k-fuel-fair-price
conda env create -f .\ai-model\03_prediction_model_design\environment-conda.yml
conda activate k-fuel-stage3
python .\ai-model\03_prediction_model_design\03_prediction_model_design.py
```

## 입력

코드의 기본 입력 경로는 다음입니다.

```text
ai-model/02_spatial_grid_build/outputs/grid.parquet
```

AI 02 Colab 노트북의 실제 최종 저장 위치는 `ROOT_PATH/그리드/grid.parquet`입니다. 로컬 03 실행 전에는 해당 파일을 위 기본 경로로 복사하거나, 코드 상단의 `GRID_PATH`를 실행 환경에 맞게 조정해야 합니다.

`grid.parquet`는 최소한 아래 컬럼을 포함해야 합니다.

| 컬럼 | 용도 |
|---|---|
| `date` | target date와 history window 구성 |
| `grid_id` | 격자 식별자 |
| `gasoline_price_mean`, `diesel_price_mean` | 유종별 격자 평균 가격 |
| `gasoline_station_count`, `diesel_station_count` | 유종별 station weight |
| `cell_x`, `cell_y`, `center_lon`, `center_lat` | 예측 결과 위치/패널 출력 |
| `official_land_price`, `official_price_source_date` | 공시지가 as-of 값과 사용 snapshot 날짜 |

02 최신 출력의 `grid.parquet`는 63,800,291행, 12,338개 격자, 2008-04-15 ~ 2026-06-11 기간을 포함합니다.

## 유종별 설정

코드는 휘발유와 경유를 같은 로직으로 순차 학습합니다.

| fuel key | 가격 컬럼 | station weight 컬럼 | label |
|---|---|---|---|
| `gasoline` | `gasoline_price_mean` | `gasoline_station_count` | 휘발유 |
| `diesel` | `diesel_price_mean` | `diesel_station_count` | 경유 |

실행 대상은 코드 상단의 `RUN_FUELS = ("gasoline", "diesel")`입니다.

## 예측 target

모델은 target date의 전국 평균 가격을 사용하지 않습니다. 입력으로 사용할 수 있는 t-1까지의 전국 평균/격자 가격 흐름을 보고, target date의 격자 가격이 전날 격자 가격 대비 얼마나 변하는지를 예측합니다.

```text
actual_price_lag_1d = target_date - 1일의 해당 격자 가격
price_delta_target = actual_grid_price(target_date) - actual_price_lag_1d
```

예측 후에는 아래 방식으로 target date의 격자 가격을 복원합니다.

```text
pred_grid_price = actual_price_lag_1d + pred_price_delta
```

`national_actual_price(target_date)`는 복원에 사용하지 않고, 학습/평가 결과에서 spread diagnostic을 계산할 때만 사용합니다.

## 모델 입력 구조

각 학습 행은 특정 `grid_id`, 특정 `target_date`에 대한 28일 시계열입니다.

| 설정 | 값 |
|---|---|
| sequence length | 28일 |
| target column | `price_delta_target` |
| target mode | `price_delta` |
| history 필요 조건 | 직전 29일의 가격/spread 이력이 연속 존재 |

LSTM에는 매일 변하는 시계열 feature만 넣습니다.

```text
spread_delta
actual_price_delta
national_price_delta
```

별도 latest branch에는 예측 전날 기준 상태값을 넣습니다.

```text
actual_price_lag_1d
national_price_lag_1d
spread_lag_1d
station_weight_lag_1d
```

별도 static/slow context branch에는 위치, 주유소 구성, 시설 영향권, 해상/도서 여부, 공시지가 같은 저빈도 feature를 넣습니다. 공시지가는 날짜별 as-of 값인 `official_land_price`를 사용하고, `official_price_source_date`로부터 `official_price_age_days`, `official_price_source_year`를 파생해 값이 얼마나 오래된 snapshot인지 함께 제공합니다.

```text
cell_x, cell_y, center_lon, center_lat
station_count_total, gasoline_station_count, diesel_station_count
brand_count__*, self_count__*
station_neighbor_influence
facility_count_total, facility_storage_count, facility_factory_count, facility_agency_count
storage_influence, agency_influence, factory_influence
is_sea, is_island, land_component_id
official_land_price, official_price_age_days, official_price_source_year
```

결측이 있거나 history window가 끊긴 행은 학습/평가 frame에서 제외합니다. static/slow context 결측은 정규화 후 0으로 대체하므로, 공시지가 최초 snapshot 이전처럼 값이 없는 시기도 실행은 가능합니다.
유종별 가격이 없거나 해당 유종 `station_count`가 0 이하인 격자는 학습/검증/test 평가 후보에 포함하지 않습니다.

## 정책기간 제외

학습/검증/final train에서는 정책기간 영향을 제거합니다.

제외 방식:

| 대상 | 처리 |
|---|---|
| `target_date`가 정책기간인 행 | 제외 |
| 29일 input history 안에 정책기간이 포함된 행 | 제외 |
| 2026년 test 행 | 학습에는 사용하지 않고 평가 전용 |

정책기간은 코드 상단의 `POLICY_EXCLUDE_RANGES`에 고정되어 있습니다.

## Train / Validation / Test 분리

| 구분 | 조건 |
|---|---|
| train/validation 후보 | `target_date < 2026-01-01` |
| test | `2026-01-01 <= target_date <= 2026-12-31` |
| train/validation split | pre-2026 eligible day를 시간순 7:3 비율로 분리 |
| gap | train 종료일과 validation 시작일 사이 7일 gap |
| final train | 이미 로드한 train/validation 샘플과 gap 샘플을 합쳐 사용 |

row 수가 매우 크기 때문에 hash 기반 고정 샘플링과 최대 row 제한을 사용합니다.

| 설정 | 값 |
|---|---:|
| `TRAIN_SAMPLE_PER_MILLE` | 200 |
| `VALID_SAMPLE_PER_MILLE` | 200 |
| final train sampling | `TRAIN_SAMPLE_PER_MILLE`와 동일 |
| `MAX_TRAIN_ROWS_PER_FUEL` | 6,000,000 |
| `MAX_VALID_ROWS_PER_FUEL` | 2,500,000 |
| `MAX_FINAL_TRAIN_ROWS_PER_FUEL` | 8,000,000 |

샘플링은 `hash(grid_id, target_date, salt)` 기반이라 시간이나 특정 지역만 잘리지 않고 재현 가능한 표본을 만듭니다. final train은 메모리를 아끼기 위해 pre-2026 전체 후보를 다시 SQL로 읽지 않고, train/validation에 이미 로드된 샘플과 7일 gap 구간 샘플을 재사용합니다.

## 중간 데이터 cache

DuckDB로 `grid.parquet`를 읽어 lag, spread, 정책기간 제외, 28일 sequence 조건을 계산하는 과정이 오래 걸리므로 중간 frame을 parquet로 cache합니다.

```text
ai-model/03_prediction_model_design/outputs/intermediate_data/
```

cache 재사용 여부는 metadata JSON과 아래 조건을 비교해 결정합니다.

| 조건 | 내용 |
|---|---|
| 입력 grid | 경로, 파일 크기, 수정 시각 |
| fuel | `gasoline` 또는 `diesel` |
| frame label | train, validation, final_train, test 월별 frame 등 |
| 날짜 범위 | start/end |
| 샘플링 | 비율, salt, 최대 row 수 |
| sequence/latest/static feature | 길이, 입력 컬럼, target 정의 |
| 정책 제외 | target/history 정책기간 제외 여부와 정책기간 목록 |
| dataset version | `stage03_price_delta_hybrid_lstm_v1` |

조건이 바뀌면 기존 cache는 사용하지 않고 새로 생성합니다.

validation score, validation prediction, training history, best checkpoint가 모두 존재하면 해당 validation 단계는 재실행하지 않고 저장된 산출물을 재사용합니다.

test는 샘플링하지 않습니다. 2026년 안에서 유종별 가격과 station count가 있고 28일 입력 시계열을 만들 수 있는 모든 행을 월별로 나눠 예측합니다.

## 모델

현재 모델은 PyTorch hybrid bidirectional LSTM입니다. 매일 변하는 28일 변화량은 LSTM으로 처리하고, 전날 기준 상태값과 static/slow context는 각각 MLP branch로 처리한 뒤 concat해 최종 price delta를 예측합니다.

| 항목 | 값 |
|---|---|
| sequence 입력 | 28 days x 3 channels |
| latest 입력 | 4 features |
| static/slow 입력 | grid schema 기준 자동 구성 |
| hidden size | 128 |
| layers | 2 |
| dropout | 0.20 |
| loss | station count weighted Smooth L1 |
| optimizer | AdamW |
| scheduler | ReduceLROnPlateau |
| max epochs | 20 |
| early stopping patience | 5 |
| LR reduce patience | 2 |
| early stopping 기준 | validation weighted MAE |

validation weighted MAE가 개선될 때마다 checkpoint를 즉시 저장합니다. 따라서 실행 중간에 중단되어도 마지막 best checkpoint는 남습니다.

baseline도 함께 계산합니다.

| baseline | 의미 |
|---|---|
| `baseline_yesterday_grid_price` | 전일 격자 가격이 그대로 유지된다고 보는 기준 |
| `baseline_train_mean_price_delta` | train 평균 가격 변동을 전일 격자 가격에 더하는 기준 |

## 예상 산출물

학습이 완료되면 `outputs/` 아래에 유종별 폴더가 생성됩니다.

```text
ai-model/03_prediction_model_design/outputs/gasoline/
ai-model/03_prediction_model_design/outputs/diesel/
```

유종별 예상 산출물은 다음입니다.

| 파일 | 내용 |
|---|---|
| `{fuel}_training_history.csv` | epoch별 train loss, validation metric |
| `{fuel}_validation_scores.csv` | baseline과 hybrid LSTM validation 성능 비교 |
| `{fuel}_validation_predictions.parquet` | validation 예측 결과 |
| `model/checkpoints/{fuel}_best_epoch_*.pt` | validation 성능이 갱신된 epoch별 best checkpoint |
| `model/checkpoints/{fuel}_best_latest.pt` | 가장 최근 best checkpoint |
| `model/{fuel}_price_delta_lstm.pt` | final train으로 학습한 최종 모델 가중치 |
| `{fuel}_model_metadata.json` | split, 입력 컬럼, 정책 제외, 성능 요약 등 실행 metadata |
| `test_predictions_by_month/YYYYMM.parquet` | 2026년 test 예측 결과 월별 파일 |
| `{fuel}_test_predictions_2026.parquet` | 2026년 test 예측 결과 통합 |
| `{fuel}_test_daily_summary_2026.csv` | 2026년 일별 예측 요약 |
| `{fuel}_test_grid_summary_2026.csv` | 2026년 격자별 예측 요약 |
| `{fuel}_test_metrics_2026.csv` | 월별 및 전체 test metric |

전체 실행이 끝나면 아래 통합 요약도 저장합니다.

```text
ai-model/03_prediction_model_design/outputs/model_run_summary.csv
```

## 현재 상태

이 README는 모델 학습 결과가 나오기 전 기준입니다. 따라서 validation/test 성능, 최종 모델 비교, 2026년 예측 해석은 아직 작성하지 않았습니다. 학습 완료 후에는 `model_run_summary.csv`, `{fuel}_model_metadata.json`, `{fuel}_test_metrics_2026.csv`를 기준으로 결과 섹션을 추가해야 합니다.
