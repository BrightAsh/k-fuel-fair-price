...
AI 04는 AI 03의 격자별 적정가격 target dataset으로 최종 예측 모델을 학습한 단계입니다. 최종 모델은 LSTM sequence branch, latest branch, static/slow branch를 결합한 hybrid 구조이며, 전날까지의 정보만으로 다음 날 격자별 적정가격을 예측합니다.
...
| 휘발유 | LSTM | 19.318 | 최종 채택 모델. validation에서는 전일 실제가격 baseline보다 나쁨 |
| 경유 | 전일 실제가격 baseline | 14.807 | 단순히 어제 가격을 오늘 적정가격으로 보는 기준 |
| 경유 | train 평균 delta baseline | 16.469 | 평균 변화량만 더하는 기준 |
| 경유 | LSTM | 19.422 | 최종 채택 모델. validation에서는 전일 실제가격 baseline보다 나쁨 |
...
이 모델을 최종 모델로 채택합니다. 채택 이유는 입력/출력 형식이 서비스 목적과 일치하기 때문입니다. 즉 사용자가 알고 싶은 것은 target date 당일의 격자별 적정가격이고, 모델은 당일 전국 평균 가격을 보지 않고 전날까지의 시계열과 격자 context만 사용합니다.

다만 성능 해석에는 명확한 한계가 있습니다.

| 한계 | 구체적 의미 |
|---|---|
| 전일 가격 baseline보다 낮은 validation | 2017~2021 validation 구간에서는 가격 레벨이 비교적 연속적이어서, 단순히 전일 가격을 유지하는 기준이 매우 강하게 작동했습니다. |
| 2026년 3월 이후 test 오차 급증 | `grid_fair_price_target`의 전국 레벨이 크게 상승했지만, 모델 예측은 전날까지의 실제 유가 흐름 근처에 머물러 음의 bias가 커졌습니다. |
| 전국 적정가격 anchor 없음 | `data-analysis/05`가 계산한 target date의 전국 정책 적용 적정가격을 입력으로 쓰지 않기 때문에, 국제가격/정책에 따른 전국 레벨 급변을 즉시 반영하기 어렵습니다. |
| 결측 없는 28일 history 의존 | 모델 입력은 28일 연속 시계열이 필요하므로, 가격 결측 또는 정책 기간 제외로 history가 끊긴 행은 학습에서 빠집니다. |
| 공간 feature는 보정 역할 | 공시지가, 시설 영향권, 브랜드/셀프 구성은 격자 차이를 설명하지만, 전국 단위 가격 충격을 직접 만들어내는 변수는 아닙니다. |
| test는 정책 기간을 제외하지 않음 | 실제 서비스 상황을 보기 위해 2026 test에는 정책 기간을 남겼습니다. 따라서 train/validation보다 어려운 조건입니다. |

따라서 이 모델의 출력은 “최종 격자별 AI 적정가격 예측값”으로 사용하되, 큰 국제가격/정책 충격이 있는 구간에서는 오차가 커질 수 있다는 설명을 함께 제공해야 합니다.
