...
| 07 데이터 격자화 | 원본 출력 기준 land grid 396,183개, 공시지가 snapshot 9개, 최종 패널 일부 진단 | 주유소 기본 패널/영향력/시설/geo flag 생성 셀의 출력이 부족하고, 최종 산출물 schema 확인 필요 | 각 parquet의 row 수, column 수, file size, date min/max, unique grid 수. 특히 최종 `grid_station_daily_panel_500m_plus_facility_plus_geo_plus_official_price.parquet` 진단 요약 |
...
| 08 최종 패널 진단 | 원본 진단 출력 기준 63,863,732행, 41열, 날짜 2008-04-15~2026-04-07, date-grid 중복 0 | 최종 패널과 모델 입력 패널이 실제로 동일한지, 공시지가 feature를 모델에 쓸지 판단 필요 | 재실행된 `panel_diagnostic_report_for_chatgpt.txt` 전체 또는 FILE_INFO, GLOBAL_SUMMARY, TARGET_SUMMARY, MODEL_SELECTOR_SUMMARY 섹션 |
| 08 모델 결과 | 원본 출력 기준 휘발유/경유 best model과 WMAE/WRMSE 참고값 | 최종 제출/보고용 metrics, feature 수, 공시지가 feature 반영 여부 | `{fuel}_model_metadata.json`, `{fuel}_validation_scores_full_features.csv`, `{fuel}_feature_importance.csv`, `{fuel}_selected_features.json` |
