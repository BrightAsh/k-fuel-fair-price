...
다만 다음 결측 구간은 웹 표시나 다음 정책 적용 단계에서 반드시 처리해야 합니다.

- `pred_gross`, `band_low`, `band_high`에는 실제 0값이 없습니다. 다만 rolling 모델이 충분한 과거 학습 표본을 확보하기 전에는 예측값과 band가 비어 있습니다.
...
단, 초기 rolling 예측 불가 구간의 `pred_gross`/band 결측, `judge` 결측 행, 경유 benchmark 결측 기간은 후속 정책 적용 및 웹 자동화 단계에서 명시적으로 필터링하거나 별도 상태로 처리해야 합니다.
