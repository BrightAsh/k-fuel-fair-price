...
2. Python 설치
3. 웹용 데이터 변환 스크립트 실행
4. 기존 `page/public/data/latest/price_history.json`과 새 결과를 병합해 날짜 이력을 누적
5. `page/public/data/latest/*.json` 변경이 있으면 commit
...
## Backfill / Force Collection

AI 모델 완성 전후로 빈 날짜를 채울 때는 workflow_dispatch의 `as_of_date`를 지정해 강제 실행합니다.

```text
Actions > Refresh Page Data > Run workflow > as_of_date=YYYY-MM-DD
```

현재 변환 스크립트는 기존 `price_history.json`을 먼저 읽은 뒤 새 산출물을 `date + region + fuel` 기준으로 병합합니다. 따라서 과거 날짜를 강제 수집해도 이미 쌓인 이후 날짜는 삭제되지 않습니다.
