...
`데이터 현황` 탭의 AI 학습 데이터 히트맵에 사용합니다.

```text
dataset,date,region,value,unit,label
```

`dataset` 기본값은 `grid_panel_rows`, `station_count`, `facility_count`, `land_price_grid_count`입니다. 날짜가 있는 데이터는 `date`를 넣고, 날짜가 없는 snapshot 데이터는 비워도 됩니다.
